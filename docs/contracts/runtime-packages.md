...
| S3 bucket、object key、content metadata 与 body | consumer | 服务 package 的 typed composition 与调用点 | `ObjectRef`、`PutRequest`、read/write results |
...
S3 core 定义 transport-neutral 的不可变 `ObjectRef`、`Reader`、`Writer`、`Store`、`PutRequest` 与 read/write result。写入 body 必须是 `io.ReadSeeker`，由 caller 持有且 package 和 adapter 永不关闭；metadata 在构造和读取边界均使用防御性副本。读取成功返回的 `io.ReadCloser` 归 caller 所有，caller 必须关闭；非 EOF read error 与 close error 只投影稳定错误，不暴露 provider 原始文本。content type、metadata、ETag、version id 与 last-modified 均为可选值；零 optional 不产生框架默认值。独立 `Presigner` 只接受 object ref、正 expiration 与可选 upload content type，结果防御性公开 URL string 与签名要求的 headers；不提供删除、分片、回调或业务策略。
