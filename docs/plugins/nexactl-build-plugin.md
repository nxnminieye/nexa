...
Generation plugin 通过 consumer 注入的 ProjectProvider 接收 Ent schema、typed Proto/API/FrontendIR document，并绑定
明确的 generated/extensions scopes 与 RPC/API/frontend delegated tool。Ent CRUD Proto 由 framework 直接检查和生成；
Provider 不复制节点 facts。Direct generation 与
