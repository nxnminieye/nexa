...
func generateRPC(service string, input []byte) {
	...
	if err := decodeStrictJSON(input, &document); err != nil || document.APIVersion != genprotocol.APIVersion || document.Kind != genprotocol.Kind || document.ServiceID != service {
		panic("invalid protocol input")
	}
	if file, ok := crudProtocolIRFile(document, service); ok {
		generateCRUDRPC(service, input, file)
		return
	}
	if validateProtocolDocument(document, service) != nil {
		panic("invalid protocol input")
	}
...
func generateCRUDRPC(service string, input []byte, file protocolFile) {
	content := renderCRUDProtocolIRGo(file, strings.ReplaceAll(service, "-", "")+"pb")
	artifactPath := "backend/" + service + "/internal/pb/" + service + ".crud.generated.pb.go"
	write(artifactPath, content)
	encoded, err := json.Marshal(rpcResult{APIVersion: "nexa.dev/rpc-go-result/v1", Artifacts: []artifactWire{{Digest: digest(content), ID: "rpc.crud", Path: artifactPath, Role: "generated"}}, GoTestPassed: true, InputDigest: digest(input), Kind: "RPCGoResult", ServiceID: service})
	if err != nil {
		panic(err)
	}
	if _, err := os.Stdout.Write(encoded); err != nil {
...
func crudProtocolIRFile(document protocolDocument, service string) (protocolFile, bool) {
...
func renderCRUDProtocolIRGo(file protocolFile, packageName string) []byte {
	var result strings.Builder
	result.WriteString("// Code generated by the consumer RPC helper. DO NOT EDIT.\n// source: " + file.Path + "\n\npackage " + packageName + "\n\n")
	for _, enum := range file.Enums {
		name := crudProtocolGoName(enum.FullName)
	...
	for _, message := range file.Messages {
		name := crudProtocolGoName(message.FullName)
		fmt.Fprintf(&result, "type %s struct {\n", name)
		for _, field := range message.Fields {
			fmt.Fprintf(&result, "\t%s %s\n", crudProtocolGoName(field.FullName), crudProtocolFieldType(field, false))
		}
		result.WriteString("}\n\n")
		for _, field := range message.Fields {
			fieldName := crudProtocolGoName(field.FullName)
			getterType := crudProtocolFieldType(field, true)
			...
			fmt.Fprintf(&result, "\treturn %s\n}\n\n", crudProtocolZero(getterType))
		}
	}
	return []byte(result.String())
}

func crudProtocolFieldType(field protocolField, getter bool) string {
	base := field.Type.Name
	if field.Type.Kind == "message" {
		base = "*" + crudProtocolGoName(base)
...
func crudProtocolGoName(name string) string {
...
func crudProtocolZero(typ string) string {
