...
func TestRenderCRUDProtocolIRGoUsesProtobufFieldNames(t *testing.T) {
	file := protocolFile{
	...
	output := string(renderCRUDProtocolIRGo(file, "accountspb"))
	parsed, err := parser.ParseFile(token.NewFileSet(), "generated.go", output, parser.AllErrors)
	if err != nil {
		t.Fatal(err)
	}
