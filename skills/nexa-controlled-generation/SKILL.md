...
description: Use when modifying or reviewing Nexa typed generation facts, direct RPC/API generation, generated scopes, extension scopes, or replacement behavior.
---
...
官方 generation plugin 只提供两个 public command 和两个 capability：
...
consumer 的 `ProjectProvider` 返回 `ServiceProject`。每个被选择的 RPC/API target 必须显式提供：

- 已解析的 typed Proto 或 API document；
- 与 provider descriptor 完全一致的 delegated tool identity；
- 唯一 generated scope；
- consumer-owned extensions、hooks、slots 或 actions scopes。
- 可选的准确 user-logic 初始文件；缺失时创建一次，已存在时默认跳过。
...
扫描或历史 manifest。声明的 user-logic 文件缺失时创建、已存在时默认跳过且字节不变；只有公开的
`--overwrite-logic` 参数为 true 时覆盖这些准确目标。其他 generated scope 之外的内容因不在写集内保持不变。
...
1. typed Proto/API 输入可由正式 parser 读取；
2. 非法路径在首次写入前失败；
...
8. 生成 Proto/API/Go 通过 parser、format、compile、unit 和真实 external-consumer E2E。
